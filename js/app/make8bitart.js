...
$(function() {
  ...
  var DOM = {
    ...
    $imageWidthInput : $('.uploader-width-input'),
    $dropHere : $('.drop-here'),
  ...
  var onDrop = function(e) {
    var palette = colorHistory;
    if (palette.length === 0) {
      // these are the default colors available in the picker
      palette = ['ff0000', 'ffa500', 'ffff00', '008000', '0000ff', '800080', '000000', '808080', 'ffffff'];
    }
    var colors = [];
    ...
    if (files.length > 1) {
      alert('Use only one file, please');
      return;
    }
    var file = files[0];
        ...
        image.onload = function() {
          ...
          for (var i = 0; i < h; i++) {
            for (var j = 0; j < w; j++) {
              ...
              drawPixel(j * pixel.size, i * pixel.size, hexToRgba(color), pixel.size);
            }
          }
        };
  ...
  DOM.$imageWidthInput.change(function() {
    DOM.$imageWidthInput.val($(this).val());
  });
  ...
  DOM.$dropHere[0].addEventListener('dragover', onDragOver, false);
  DOM.$dropHere[0].addEventListener('drop', onDrop, false);
